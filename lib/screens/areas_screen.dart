...
import 'package:lekapp_supervisor/screens/zones_screen.dart';
...
class AreasScreen extends StatefulWidget {
  ...
  AreasScreen({required this.buildingSite, required this.speciality}) : super();
    ...
    for (Activity activity in snapshot.data!.activities) {
      if (activity.fk_speciality == speciality.id) {
        ...
        for (Zone zone in snapshot.data!.zones) {
          if (zone.id == activity.fk_zone) {
            for (Area area in snapshot.data!.areas) {
              if (area.id == zone.fk_area) {
    ...
    if (areas.isEmpty) {
      return ListView(
        children: <Widget>[
          Center(
            child: Text(
              'No hay áreas para esta especialidad',
            ...
            if (snapshot.hasData) {
              ...
              children = const <Widget>[
                ...
                Padding(
                  ...
                  child: Text('Cargando áreas disponibles...'),
                )
              ];
              result = Center(
                child: Column(
